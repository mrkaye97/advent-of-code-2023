...
def determine_num_winning_charge_times(race: tuple[str, str]) -> int:
    ...
    low = floor((-1 * race_duration + sqrt(race_duration**2 - 4 * (-1) * (-1 * race_record_distance))) / (-2))
    high = ceil((-1 * race_duration - sqrt(race_duration**2 - 4 * (-1) * (-1 * race_record_distance))) / (-2))
